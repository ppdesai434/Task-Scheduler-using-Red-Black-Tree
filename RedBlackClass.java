...
public class RedBlackClass {
    
    public TreeNode root;
    public DrawingFrame frameobject=null;
    private class TreeNode{
        private Task value;
        private int unfairness;
        private TreeNode parentNode;
        private TreeNode leftchild, rightchild;
        
        private boolean color;
        public TreeNode(Task tvalue, int tunfairnessvalue){
            this.value=tvalue;
            this.unfairness=tunfairnessvalue;
            this.leftchild=null;
            this.rightchild=null;
            this.parentNode=null;
        }
    }
    private static final boolean REDCLR = true;
    private static final boolean BLACKCLR = false;
    public static int countcompleted=1;
    ...
    public void insertNode(Task tvalue, int unfairnessvalue){
        if(this.nodeNumber==0){
            ...
            while(!isRightPlaceFound){
                ...
                if(temp.unfairness>=createdNode.unfairness){
            ...
            if(uncle == null){
                ...
                if( (papa !=null) && (grandPa != null) && (uncle!=null)){
                    if(isRed(papa) && isRed(uncle)){
                        papa.color = BLACKCLR;
                        uncle.color = BLACKCLR;
                        if(grandPa!=root){
                            grandPa.color=REDCLR;
...
//        }
    ...
    public void drawTree(TreeNode tnode,int x, int y){
        ...
        CircleClass a=new CircleClass(tnode.color, x, y, tnode.value.taskId, tnode.unfairness);
    ...
    private void printValue(TreeNode x){
            if(x.color==BLACKCLR){
                System.out.print(x.value.taskId+"("+x.unfairness+")"+"-");
            }else{
                System.out.print(x.value.taskId+"("+x.unfairness+")"+"*");
            }
    ...
    public void deleteMinServeAddAgain(){
        ...
        if(isRed(leftMostNode)){
            ...
            //serve the task:
            leftMostNode.value.unfairness++;
            //add that node again if it requires more cpu time
            if(leftMostNode.value.timeRequired > leftMostNode.value.unfairness)
                insertNode(leftMostNode.value, leftMostNode.value.unfairness);
            else
            	countcompleted++;
    ...
    public void deleteMinWhichIsBlack(TreeNode x){
        boolean xyz=false;
        if(x.value.taskId==3&&x.unfairness==10){
            xyz=true;
        }
        ...
        if(x==root){
            ...
            this.nodeNumber = this.nodeNumber - 1;
            x.value.unfairness++;
            if(x.value.timeRequired > x.value.unfairness)
                insertNode(x.value, x.value.unfairness);
    ...
    public void drawJFrame(){
        if(frameobject==null){
            frameobject = new DrawingFrame("Task Scheduler Snapshot for Red black tree");
